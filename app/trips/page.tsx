    ...
    <PageShell locale={locale} user={user}>
      <div className="mx-auto max-w-6xl px-4 py-10">
        ...
        <form className="mt-6 flex flex-wrap gap-3">
          ...
          <button className="rounded-full bg-pine px-4 py-2 text-sand">Filter</button>
        </form>
          ...
          {trips.map((trip) => {
            ...
            return (
              <Link
                key={trip.id}
                href={`/trips/${trip.id}`}
                className="card grid gap-4 rounded-3xl p-5 transition hover:-translate-y-0.5 sm:grid-cols-[1fr_auto]"
              >
                <div>
